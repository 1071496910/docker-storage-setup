...
reset_lvm_thin_pool () {
  ...
  if  $(lvs | grep -q 'data ') ;then
      lvremove -f /dev/$VG/data
  fi
  
  if  $(lvs | grep -q 'data-pool') ;then
      lvchange -an $VG/data-pool
      lvremove -f $VG/data-pool
  fi
...
canonicalize_block_devs() {
  ...
  for dev in ${devs}; do
    ...
    if is_block_dev_partition ${dev_abs}; then
      : #echo "Fatal" "Partition specification unsupported at this time."
    fi
...
EOF
  ...
  if ! wait_for_dev "${dev}p1"; then
    Fatal "Partition device ${dev}p1 is not available"
  fi
}
...
setup_data_volume(){
if ! $(lvs | grep -q data-pool) ;then
  lvcreate -L 100G --thinpool data-pool $VG
  local profileName=/etc/lvm/profile/$VG--data-pool-extend.profile
  cat <<EOF >${profileName}
activation {
...
EOF
  ...
  lvchange --metadataprofile $VG--data-pool-extend  $VG/data-pool
  lvcreate -V 300G --thin -n data $VG/data-pool 
  mkfs.xfs /dev/$VG/data 
fi
if [ ! -d /data ];then
  mkdir  /data
fi
if ! $(mount | awk '{if($3=="/data"){print $3}}'|grep -q '/data');then
  mount /dev/$VG/data /data/
fi
}

#####remove_wrong_fstable
remove_wrong_fstable(){
if $(cat /etc/fstab | grep -v '#' | awk '{if($2=="/data"){print $2}}'|grep -q '/data') ;then
...
for DEV in $DEVS
do
    if ! $(pvs --no-head | grep -q $DEV);then
        pvcreate -f $DEV
    fi
done
    if ! $(vgs --no-head | grep -q $VG);then
    	vgcreate $VG $DEVS
    fi
}
